...
const styles = StyleSheet.create({
    form: { flex: 1, paddingTop: 50, paddingLeft: 20, paddingRight: 20, justifyContent: 'space-between' },
    fieldStyles: { height: 70, color: '#f47100' },
    addButton: { marginTop: 20 },
});

class UpdatePerson extends Component {
    onUpdatePress() {
        const { firstName, lastName, phone, email, company, school, project, notes, employee, _id } = this.props;
        this.props.saveContact({ firstName, lastName, phone, email, company, school, project, notes, employee, _id });
    }
...
const mapStateToProps = state => {
    const { firstName, lastName, phone, email, company, school, project, notes, employee, _id } = state;
    return { firstName, lastName, phone, email, company, school, project, notes, employee, _id };
};
